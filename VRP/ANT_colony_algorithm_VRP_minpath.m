%Сначала осуществляется поиск минимального маршрута(задача коммивояжера), а затем мы разбиваем
%  общий минимальный маршрут на циклические подмаршруты(от базы -
%   по городам - обратно на базу).
function [ Route, Length_Route ] = ANT_colony_algorithm_VRP_minpath( ...
...
%%
%         Алгоритм разбивки общего маршрута на подмаршруты
%_________________________________________________________________
%Для VRP: Т.к. мы нашли минимальный маршрут. В коде ниже мы разбиваем
%  общий минимальный маршрут на циклические подмаршруты.(от базы -
%   по городам - обратно на базу).
%_________________________________________________________________

Route(1:end) = Route(1:end) + 1; %номер каждого пункта уведичиваем на 1,
                                 %  т.к. мы решали задачу коммивояжера,
                                 %  начиная с 1 по dim пункт, а 1ый пункт -
                                 %  это наша база
...
nvehicle = 1; %выбираем первое ТС
ind_start = 1; %индекс стартового пункта в общем маршруте для записи в маршрут ТС
Route(end) = [];
q2 = 0;
temp_dim = dim + 1;
for i = 1:temp_dim + 1  
    ...
    if q1 + q2 <= c
        ...
        Route(indx) = 1;
        vehicles(nvehicle).route = [vehicles(nvehicle).route Route(ind_start:indx)]; %добавляем маршрут для ТС
        ind_start = indx + 1;
        nvehicle = get_No_vehicle(vehicles); %получаем номер следующего ТС
        c = vehicles(nvehicle).capacity;     % и его грузовместимость
        Route = [Route,R2];
        i = i + 1;
        temp_dim = temp_dim + 1;
        q2 = 0; 
...
%%
vehicles(nvehicle).route = [vehicles(nvehicle).route Route(ind_start:end) 1]; %добавляем последний подмаршрут общего маршрута в маршрут для ТС
...
function [nvehicle] = get_No_vehicle(vehicles)
    dim = length(vehicles);
    %ищем следующее ТС
    N_of_bases_in_routes = zeros(1,dim); %количество баз в каждом маршруте ТС
    for i = 1:dim
