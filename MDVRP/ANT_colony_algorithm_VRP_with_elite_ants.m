...
% маршруты на подмаршруты, исходя из грузовместимости ТС, при этом
% используя элитных муравьев, которые усиливают маршруты с наименьшей
% длиной
function [ Route, Length_Route, vehicles ] = ANT_colony_algorithm_VRP_with_elite_ants( ...
...
savings = create_savings(distances); %матрица выигрышей
...
N_elite_ants = param(5); %количество элитные муравьи
...
for iter = 1: iter_max
    ...
    % выделяем лучшие маршруты и (n_N_elite_ants-1) количество элитных муравьев    
    [best_routes, IND_best] = sort(length_route_ants);
    if N_elite_ants < N_ants
        for elite = 1: N_ants - (N_elite_ants - 1)
            best_routes(end) = []; 
            IND_best(end) = [];
        end
    end
    ...
    for e_ant = 1: N_elite_ants - 1
        delta_tau = zeros(dim); %количество феромона на каждом из ребер
        for w = 1:length(route_ants(IND_best(e_ant), :)) - 1
            Ri = route_ants(IND_best(e_ant), w); Ri(Ri == 0) = [];
            Rj = route_ants(IND_best(e_ant), w + 1); Rj(Rj == 0) = [];
            i = Ri;
            j = Rj;
            delta_tau(i,j) = (N_elite_ants - e_ant)/best_routes(e_ant);
        end
        sum_delta_tau = sum_delta_tau + delta_tau;
    end
    length_best_route = best_routes(1);
    delta_tau_star = zeros(dim);
    for e = 1:length(route_ants(IND_best(1), :)) - 1
        Ri = route_ants(IND_best(1), e); Ri(Ri == 0) = [];
        Rj = route_ants(IND_best(1), e + 1); Rj(Rj == 0) = [];
        i = Ri;
        j = Rj;
        delta_tau_star(i,j) = 1/length_best_route;
    end
    tau = evaporation_fer * tau + sum_delta_tau + N_elite_ants * delta_tau_star;
    tau(1,1) = 0;
    %-------------------------------------------------------------------
    route_ants(:,:) = 0;
end
